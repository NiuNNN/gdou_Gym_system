<template>
  <div class="descriptions-container">
    <div class="descriptions-box">
      <el-page-header @back="goBack" content="修改管理员资料" title="返回主页">
      </el-page-header>
      <!-- 用户资料 -->
      <div class="left">
        <el-descriptions
      ...
      <div class="btn">
        <el-button type="primary" @click="edit"
          ><i class="el-icon-edit"></i>修改资料</el-button
        >
      </div>
    </div>
    <!-- 编辑弹窗层 -->
    <div class="edit-form">
      <el-dialog title="修改资料" :visible.sync="dialogFormVisibleEdit">
        <h1 class="usercode">账号:{{ user.code }}</h1>
        <div class="upload">
          <el-upload
            :action="avatarUrl"
...
export default {
  ...
  data() {
    ...
    return {
      ...
      rules: {
        ...
        sex: [{ validator: validateSex, trigger: 'blur', required: true }],
  ...
  methods: {
    ...
    getuserinfo() {
      ...
      console.log(userid);
      request({
        url: '/superusers/toGetSuperUserInfo/' + userid,
        methods: 'Get',
      }).then((res) => {
    ...
    getImgUrl() {
      let userid = localStorage.getItem('userid')
      this.user.avatar =
        'http://localhost/avatar/' + userid + '.jpg' + '?' + Math.random() * 10
  ...
  computed: {
    avatarUrl() {
      let userid = localStorage.getItem('userid')
      let avatarUrl =
        'http://localhost:80/users/toUploadUserAvatar?code=' + userid
      return avatarUrl
    },
  },
}
